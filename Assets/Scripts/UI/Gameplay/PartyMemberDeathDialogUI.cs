...
{
    public sealed class PartyMemberDeathDialogUI : MonoBehaviour
    {
        const string Title = "Party member fallen";

        static PartyMemberDeathDialogUI _instance;
        ...
        public static PartyMemberDeathDialogUI EnsureInstance()
        {
            ...
            var go = new GameObject(nameof(PartyMemberDeathDialogUI));
            DontDestroyOnLoad(go);
            _instance = go.AddComponent<PartyMemberDeathDialogUI>();
            return _instance;
        }
        ...
        public void Show(string memberDisplayName, Action onOk)
        {
            ...
            string name = string.IsNullOrWhiteSpace(memberDisplayName) ? "Party member" : memberDisplayName.Trim();
            if (_bodyText != null)
            {
                _bodyText.text =
                    $"<b>{name}</b> has died.\n\n<size=14>HP reached zero.</size>";
            }
        ...
        void EnsureModalBuilt()
        {
            ...
            var canvasGo = new GameObject("PartyDeathDialogCanvas",
            ...
            bubbleRt.sizeDelta = new Vector2(480f, 260f);
