...
{
    public sealed class AdjacentInteractPickerModalUI : MonoBehaviour
    {
        static AdjacentInteractPickerModalUI _instance;
        ...
        readonly List<IAdjacentMapInteractable> _targets = new List<IAdjacentMapInteractable>();
        BaseActor _actor;
        Action<IAdjacentMapInteractable> _onSelected;
        int _focusIndex;
        bool _blocking;

        public static bool BlocksGameplay => _instance != null && _instance._blocking;

        public static AdjacentInteractPickerModalUI EnsureInstance()
        {
            if (_instance != null)
                return _instance;

            var go = new GameObject(nameof(AdjacentInteractPickerModalUI));
            DontDestroyOnLoad(go);
            _instance = go.AddComponent<AdjacentInteractPickerModalUI>();
            return _instance;
        }
        ...
        public void Show(
            BaseActor actor,
            IReadOnlyList<IAdjacentMapInteractable> targets,
            Action<IAdjacentMapInteractable> onSelected)
        {
            EnsureModalBuilt();
            _actor = actor;
            ...
            if (_titleText != null)
                _titleText.text = "INTERACT";

            if (_hintText != null)
            {
                _hintText.text =
                    "<size=14><color=#9bbdff>Enter</color> choose   ·   "
        ...
        void ConfirmFocused()
        {
            ...
            IAdjacentMapInteractable selected = _targets[_focusIndex];
            Action<IAdjacentMapInteractable> cb = _onSelected;
            Close();
            cb?.Invoke(selected);
        }
        ...
        void RefreshListText()
        {
            ...
            if (_targets.Count == 0)
            {
                _listText.text = "Choose what to interact with:\n\n  (none)";
                return;
            }

            var sb = new System.Text.StringBuilder();
            sb.AppendLine("Choose what to interact with:");
            sb.AppendLine();
            ...
            for (int i = 0; i < _targets.Count; i++)
            {
                ...
                sb.Append(prefix);
                sb.AppendLine(_targets[i].ListLabel);
            }
