...
{
    public sealed class TrapConfirmDialogUI : MonoBehaviour
    {
        static TrapConfirmDialogUI _instance;
        ...
        public static TrapConfirmDialogUI EnsureInstance()
        {
            if (_instance != null)
                return _instance;

            var go = new GameObject(nameof(TrapConfirmDialogUI));
            DontDestroyOnLoad(go);
            _instance = go.AddComponent<TrapConfirmDialogUI>();
            return _instance;
        }
        ...
        public void Show(BaseActor mover, TrapDefinition trap, Action onYes)
        {
            EnsureModalBuilt();
            _onYes = onYes;
            _blocking = true;
            ...
            if (_bodyText != null)
            {
                ...
                _bodyText.text =
                    $"{actorName} is about to enter a tile with a visible {trapName}. Move onto this tile anyway?\n\n" +
        ...
        void EnsureModalBuilt()
        {
            ...
            var canvasGo = new GameObject("TrapConfirmCanvas",
